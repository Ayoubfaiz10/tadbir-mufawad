...
(function () {
  ...
  async function loadTab() {
    ...
    else if (activeTab === 'tariffs') await loadTariffs();
  ...
  async function loadPayments() {
    try {
      const res = await API.payList({ page: 1, pageSize: 100 });
      renderPayments(res.rows || []);
  ...
  async function loadAssessments() {
    try {
      const res = await API.assessmentList({ page: 1, pageSize: 100 });
      renderAssessments(res.rows || []);
    } catch (e) { toast(t('finance.errors.load'), true); }
  }
  ...
  async function init() {
    try {
      config.types = (await API.configSnapshot()).types || [];
    } catch (e) { /* silent */ }
    bindTabs();
